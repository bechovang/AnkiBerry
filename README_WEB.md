...
### 4. Hệ thống Proxy Âm thanh & Hình ảnh bảo mật (Secure Media Proxying & Autoplay)
Trên các trình duyệt cũ của BlackBerry 10, chính sách bảo mật SSL nghiêm ngặt và giới hạn chia sẻ Cookie bên thứ ba khiến trình duyệt không thể tải trực tiếp các tệp âm thanh `.mp3` và hình ảnh đính kèm từ các tên miền con của AnkiWeb như `ankiuser.net`.

**Giải pháp**:
- **Media Proxy Route**: Flask proxy tự động nhận các yêu cầu tải tài nguyên âm thanh (MP3, WAV, OGG,...) và hình ảnh, sau đó gửi yêu cầu lấy trực tiếp từ máy chủ AnkiWeb bằng chính session cookie nội bộ đã xác thực, rồi truyền ngược (stream) dữ liệu nhị phân về cho BlackBerry.
- **Tự động chuyển đổi thẻ `[sound:...]`**: Backend tự động nhận diện định dạng âm thanh thô của Anki (`[sound:filename.mp3]`) và biên dịch trực tuyến sang thẻ HTML5 `<audio>` chuẩn, hiển thị một trình phát âm thanh tuyệt đẹp và tương thích tốt với WebKit cũ.
- **Tính năng Autoplay**: Tích hợp mã JavaScript tự động tìm thẻ `<audio>` bên trong ô câu hỏi ngay khi chuyển thẻ, hoặc ô đáp án ngay khi lật thẻ và kích hoạt hàm `.play()` trong khối `try...catch` an toàn. Cơ chế này đảm bảo âm thanh được phát tự động mà không làm treo hay gián đoạn luồng JavaScript của trình duyệt cũ.
